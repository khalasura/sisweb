<template>
<div>
    <b-modal id="modal-classLine2" title="결함클래스" header-bg-variant="dark" header-text-variant="light" cancel-variant="danger" @ok="handleOk" size="lg">
        ...
        clearBtn() {
            ...
            .then((result) => {
                if(result.value) {
                    ...
                    var hideList = ['System', 'Pseudo', 'Fd', 'Un', 'Ps'];
    ...
    computed: {
        ...
        count: function() {
            ...
            for(var i = 0; i < this.$store.state.chartLine2.length; i++) {
                count += this.$store.state.chartLine2[i].data.length
            }
            return count
        },
        classOp: function() {
            ...
            var str = [];
            for(var i = 0; i < this.$store.state.chartLine2.length; i++) {
                overlapStr[i] = this.$store.state.chartLine2[i].name.substr(0, 1)
            }
            overlapStr.sort();
