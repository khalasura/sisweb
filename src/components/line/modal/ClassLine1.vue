<template>
<div>
    <b-modal id="modal-classLine1" title="결함필터" header-bg-variant="dark" header-text-variant="light" cancel-variant="danger" @ok="handleOk">
...
export default {
    ...
    methods: {
        ...
        getTreeData() {
            ...
            this.$store.state.chartLine1.forEach(g => {
                items.forEach(t => {
                    if(g.group == t.text) {
    ...
    computed: {
        ...
        chartArray: function() {
            ...
            var str = [];
            var result = [];
            for(var i = 0; i < this.$store.state.chartLine1.length; i++) {
                overlapStr[i] = this.$store.state.chartLine1[i].group;
            }
            overlapStr.sort();
            ...
            str.forEach(g=>{
                var cnt = 0;
                var find = this.$store.state.chartLine1.filter(val=> val.group == g);
                find.forEach(f=>{
                    cnt += f.data.length
                });
