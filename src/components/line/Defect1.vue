        ...
        <tbody>
            <tr>
                <td style="padding: 5px; margin: 0;">
                    <label v-if="reverse1 == true" style="margin-bottom: 3px;">
                        ↓{{BoxBottom}}&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;↑{{DistanceFromStart}}
                    </label>
                    <label v-if="reverse1 == false" style="margin-bottom: 3px;">
                        ↓{{DistanceFromStart}}&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;↑{{BoxBottom}}
                    </label>
                </td>
                <td style="padding: 5px; margin: 0;">→{{DistanceLeftEdge}}
                    &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
                    ←{{DistanceRightEdge}}
                </td>
            </tr>
...
export default {
    computed: {
        DefectNumber: function() {
            if(this.$store.state.coildefect1.DefectNumber == null) {
                return '　'
            } else {
                return this.$store.state.coildefect1.DefectNumber
            }
        },
        Side: function() {
            if(this.$store.state.coildefect1.Side == null) {
                return '　'
            } else {
                return this.$store.state.coildefect1.Side
            }
        },
        Class: function() {
            if(this.$store.state.coildefect1.Class == null) {
                return '　'
            } else {
                return this.$store.state.coildefect1.Class
            }
        },
        RepeatLength: function() {
            if(this.$store.state.coildefect1.RepeatLength == null) {
                return '　'
            } else {
                return this.$store.state.coildefect1.RepeatLength.toFixed(0).replace(/\B(?=(\d{3})+(?!\d))/g, ",")
            }
        },
        DistanceFromStart: function() {
            if(this.$store.state.coildefect1.DistanceFromStart == null) {
                return '　'
            } else {
                return (this.$store.state.coildefect1.DistanceFromStart / 1000).toFixed(1).replace(/\B(?=(\d{3})+(?!\d))/g, ",")
            }
        },
        BoxBottom: function() {
            if(this.$store.state.coildefect1.DistanceFromStart == null) {
                ...
                var sis = (this.$store.state.line1.length / 1000).toFixed(1);
                var roi = (this.$store.state.coildefect1.HeightSize / 1000).toFixed(1);
                var box = (this.$store.state.coildefect1.DistanceFromStart / 1000).toFixed(1);
                var count = sis - roi - box;
                return count.toFixed(1).toString().replace(/\B(?=(\d{3})+(?!\d))/g, ",")
            }
        },
        DistanceLeftEdge: function() {
            if(this.$store.state.coildefect1.DistanceLeftEdge == null) {
                return '　'
            } else {
                return (this.$store.state.coildefect1.DistanceLeftEdge).toFixed(0).replace(/\B(?=(\d{3})+(?!\d))/g, ",")
            }
        },
        DistanceRightEdge: function() {
            if(this.$store.state.coildefect1.DistanceRightEdge == null) {
                return '　'
            } else {
                return (this.$store.state.coildefect1.DistanceRightEdge).toFixed(0).replace(/\B(?=(\d{3})+(?!\d))/g, ",")
            }
        },
        DefectArea: function() {
            if(this.$store.state.coildefect1.DefectArea == null) {
                return '　'
            } else {
                return this.$store.state.coildefect1.DefectArea.toFixed(1).replace(/\B(?=(\d{3})+(?!\d))/g, ",")
            }
        },
        HeightSize: function() {
        ...
        reverse1: function() {
            return this.$store.state.reverseBtnClick1
        },
        Roll: function() {
            if(this.$store.state.coildefect1.RepeatLength == null) {
                return '　'
            } else {
                return (this.$store.state.coildefect1.RepeatLength / 3.14).toFixed(1).replace(/\B(?=(\d{3})+(?!\d))/g, ",")
            }
        },
        CenterLength: function() {
            if(this.$store.state.coildefect1.DistanceLeftEdge == null) {
                ...
                var defect = this.$store.state.coildefect1;
