<template>
<div>
    <b-modal id="modal-classBOT" title="결함클래스" header-bg-variant="dark" header-text-variant="light" cancel-variant="danger" @ok="handleOk" size="lg">
    ...
    computed: {
        ...
        count: function() {
            ...
            for(var i = 0; i < this.$store.state.chartBOT.length; i++) {
                count += this.$store.state.chartBOT[i].data.length
            }
            return count
        },
        classOp: function() {
            ...
            var str = [];
            for(var i = 0; i < this.$store.state.chartBOT.length; i++) {
                overlapStr[i] = this.$store.state.chartBOT[i].name.substr(0, 1)
            }
            overlapStr.sort();
...
#infoLabel{ font-size: 14px; float: left }
